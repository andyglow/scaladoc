...
class ScaladocMacro(val c: blackbox.Context) extends ExtractScaladoc {
  import c.universe._

  val carrierTpe = typeOf[ScaladocCarrier]
  val prefix = q"scaladocx"
...
//          val hasTags = args.get(TermName("tags")).get
          ...
          Scaladoc.fromString(body.get(), hasTags.get(), strict = true) match {
            case Right(doc) => doc
      ...
      for {
        ...
        scd <- Scaladoc.fromString(str, strict = true).toOption
      } yield scd
    }
